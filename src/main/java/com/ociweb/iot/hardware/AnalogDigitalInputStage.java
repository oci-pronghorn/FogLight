...
import com.ociweb.pronghorn.pipe.GroveResponseSchema;
import com.ociweb.pronghorn.stage.PronghornStage;
import com.ociweb.pronghorn.stage.scheduling.GraphManager;

public class AnalogDigitalInputStage extends PronghornStage{
	//TODO: Change the Schema
	private final Pipe<GroveResponseSchema> toListener;

	private DataOutputBlobWriter<GroveResponseSchema> writeListener;

	private Hardware hardware;

	private static final Logger logger = LoggerFactory.getLogger(JFFIStage.class);

	public AnalogDigitalInputStage(GraphManager graphManager, Pipe<GroveResponseSchema> toListener, Hardware hardware) {
		super(graphManager,NONE, toListener); 
		...
		this.writeListener = new DataOutputBlobWriter<GroveResponseSchema>(toListener);
	...
	public void run() {
		for (int i = 0; i < this.hardware.digitalInputs.length; i++) {
			if(this.hardware.digitalInputs[i].type.equals(ConnectionType.Direct)){
				if (tryWriteFragment(toListener, GroveResponseSchema.MSG_CHUNKEDSTREAM_1)) { //TODO: Do we want to open and close pipe writer for every poll?
					...
					DataOutputBlobWriter.closeHighLevelField(writeListener, GroveResponseSchema.MSG_CHUNKEDSTREAM_1_FIELD_BYTEARRAY_2);
