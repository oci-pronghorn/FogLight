	        ...
	        do {
        		...
        		do{
        			...
        			if(inProgressIdx != -1) {
                        ...
                        logger.info("i2c request read from address: {}  ",connection.address);//+Arrays.toString(Arrays.copyOfRange(connection.readCmd, 0, connection.readCmd.length)));
		...
		while ( hasReleaseCountRemaining(activePipe) 
			...
			switch(msgIdx){
    			case I2CCommandSchema.MSG_COMMAND_7:
    			{
                    ...
                    logger.info("i2c request write to address: {}  ",addr);//+Arrays.toString(Arrays.copyOfRange(connection.readCmd, 0, connection.readCmd.length)));
